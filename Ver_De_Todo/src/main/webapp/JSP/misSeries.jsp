<%-- 
    Document   : misSeries
    Created on : 13-dic-2018, 20:04:57
    Author     : aitor
--%>
            ...
            <div class="row">
                ...
                <div class="col-lg-9">
                    <div class="row my-4" id="contenido">
                        <c:if test="${sessionScope.misSeries.size() == 0 || sessionScope.misSeries == null}">
                            <div class="col-lg-12 mb-4">
                                <h2>Aun no has subido ninguna serie a tu canal</h2>
                            </div>
                        </c:if>
                        <c:forEach var="miSerie" items="${sessionScope.misSeries}">
                            <div class="col-lg-4 mb-4" style="float:left;">
                                <div class="card h-100">
                                    <a href="../Controlador?direccion=temporadasSerie&nombreSerie=${miSerie.tituloSerie}">
                                        <img class="card-img-top" src="http://127.0.0.1:8887/${miSerie.nombrePortada}" alt="Image" style="max-height: 321.19px; min-height: 321.19px;">
                                    </a>
                                    <div class="card-body" style="min-height: 170px; max-height: 170px;">
                                        <h4 class="card-title">
                                            <a href="#">${miSerie.tituloSerie}</a>
                                        </h4>
                                        <small class="text-muted">Idioma: ${miSerie.idioma}</small><br>
                                        <c:if test="${miSerie.subtitulos != ''}">
                                            <c:if test="${miSerie.subtitulos == 'L'}">
                                                <small class="text-muted">Subtitulos: Latino</small>
                                            </c:if>
                                            <c:if test="${miSerie.subtitulos == 'E'}">
                                                <small class="text-muted">Subtitulos: Español</small>
                                            </c:if>
                                            <c:if test="${miSerie.subtitulos == 'I'}">
