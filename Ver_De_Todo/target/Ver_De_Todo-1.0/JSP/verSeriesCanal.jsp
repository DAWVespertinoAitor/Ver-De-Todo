<%-- 
    Document   : verSeriesCanal
    Created on : 13-dic-2018, 23:20:41
    Author     : aitor
--%>
            ...
            <div class="row">
                ...
                <div class="col-lg-9">
                    <div class="row my-4" id="contenido">
                        <c:if test="${sessionScope.seriesCanal.size() == 0}">
                            <div class="col-lg-12 mb-4">
                                <h2>${sessionScope.usuarioVerCanal.nombreCanal} aun no ha subido ninguna serie.</h2>
                            </div>
                        </c:if>
                        <c:if test="${sessionScope.seriesCanal.size() != 0}">
                            <div class="col-lg-12 mb-4">
                                <h2>Series del canal ${sessionScope.usuarioVerCanal.nombreCanal}</h2>
                            </div>
                        </c:if>
                        <c:forEach var="seriesCanal" items="${sessionScope.seriesCanal}">
                            <div class="col-lg-4 mb-4" style="float:left;">
                                <div class="card h-100">
                                    <a href="../Controlador?direccion=temporadasSerieCanal&nombreSerieCanal=${seriesCanal.tituloSerie}">
                                        <img class="card-img-top" src="http://127.0.0.1:8887/${seriesCanal.nombrePortada}" alt="Image" style="max-height: 321.19px; min-height: 321.19px;">
                                    </a>
                                    <div class="card-body" style="min-height: 170px; max-height: 170px;">
                                        <h4 class="card-title">
                                            <a href="#">${seriesCanal.tituloSerie}</a>
                                        </h4>
                                        <small class="text-muted">Idioma: ${seriesCanal.idioma}</small><br>
                                        <c:if test="${seriesCanal.subtitulos != ''}">
                                            <c:if test="${seriesCanal.subtitulos == 'L'}">
                                                <small class="text-muted">Subtitulos: Latino</small>
                                            </c:if>
                                            <c:if test="${seriesCanal.subtitulos == 'E'}">
                                                <small class="text-muted">Subtitulos: Español</small>
                                            </c:if>
                                            <c:if test="${seriesCanal.subtitulos == 'I'}">
